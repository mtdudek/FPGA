...
module Avalon_Master_Memory (
	...
	_edge_detector det0(key0_edge,clock,key2[0]),det1(key1_edge,clock,key2[1]),
						det2(key2_edge,clock,key2[2]),det3(key3_edge,clock,key2[3]);
	...
	always @(posedge clock or posedge reset)begin
		if(reset)begin
			...
			if(key0_edge)begin 
				...
				refresh <= 1;
				counter <= 0;
			end else  if(key3_edge) begin
				avalon_master_writedata <= sw2[0+:8];
				write <= 1;
			end
			if (refresh) begin
				avalon_master_read <= 1'b1;
				avalon_master_chipselect <= 1'b1;
				if (!avalon_master_waitrequest) begin
					avalon_master_read <= 1'b0;
					avalon_master_chipselect <= 1'b0;
					mem_out<= avalon_master_readdata;
					counter <= 0;
					refresh <= 0;
				end
			end 
